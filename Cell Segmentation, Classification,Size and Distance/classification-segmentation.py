...
def display_and_save_combined(image_path, cellpose_model, yolo_model, save_path):
    ...
    # Match YOLO detections to segmentation
    ...
    if len(seg_points) > 0 and len(label_points) > 0:
        distances = cdist(label_points, seg_points)
    ...
    # Assign "RBC" to all unassigned segments
    ...
    for i, outline in enumerate(outlines):
        ...
        ax.plot(outline[:, 0], outline[:, 1], color='red', linewidth=1.5)
        ax.text(cx, cy, label, color='yellow', fontsize=6, weight='bold',
...
input_folder = '/content/drive/MyDrive/test_images'   #Change this to your input folder
output_folder = '/content/drive/MyDrive/seg_class_output' #Change this to your output folder
